...
func (s *Server) updateOurFederationState(curr *structs.FederationState) error {
	...
	if s.config.Datacenter == s.config.PrimaryDatacenter {
		...
		_, err := s.leaderRaftApply("FederationState.Apply", structs.FederationStateRequestType | structs.IgnoreUnknownTypeFlag, args)
		if err != nil {
			return err
		}
	} else {
...
func (s *Server) pruneStaleFederationStates() error {
	...
	for _, fedState := range fedStates {
		...
		_, err := s.leaderRaftApply("FederationState.Delete", structs.FederationStateRequestType | structs.IgnoreUnknownTypeFlag, &req)
