...
You can configure some [L7 features](/consul/docs/manage-traffic) in [configuration entries](/consul/docs/fundamentals/config-entry). You can add [custom Envoy configurations](#advanced-configuration) to the [proxy service definition](/consul/docs/reference/proxy/connect-proxy), which enables you to leverage Envoy features that are not exposed through configuration entries. You can also use the [Consul Envoy extensions](/consul/docs/envoy-extension) to implement Envoy features.
...
- The 2 latest [Consul Enterprise LTS](/consul/docs/upgrade/lts) major releases
...
However, [Consul Enterprise Long Term Support (LTS)](/consul/docs/upgrade/lts)
...
[Intentions](/consul/docs/secure-mesh/intention) are enforced using Envoy's RBAC filters. Depending on the
configured [protocol](/consul/docs/reference/config-entry/service-defaults#protocol) of the proxied service, intentions are either enforced
...
Consul Dataplane automatically configures and manages an Envoy process. Consul Dataplane generates the Envoy bootstrap configuration file prior to starting Envoy. To configure how Consul Dataplane starts Envoy, refer to the [Consul Dataplane CLI reference](/consul/docs/reference/dataplane/cli).
...
entry](/consul/docs/reference/config-entry/proxy-defaults) or override them directly in the `proxy.config`
field of a [proxy service definition](/consul/docs/reference/proxy/connect-proxy). When
connected to a Consul client agent, you can place the configuration in the `proxy.config` field of
the [`sidecar_service`](/consul/docs/connect/proxy/sidecar) block.
...
- `envoy_statsd_url` - A URL in the form `udp://ip:port` identifying a UDP
  ...
  configuration entry](/consul/docs/reference/config-entry/proxy-defaults) for convenience. Currently, TCP is not supported.
  ...
  configuration entry](/consul/docs/reference/config-entry/proxy-defaults). The env
...
entry](/consul/docs/reference/config-entry/service-defaults). Consul will use this
...
entry](/consul/docs/reference/config-entry/service-defaults). Agents with
[`enable_central_service_config`](/consul/docs/reference/agent/configuration-file/general#enable_central_service_config)
...
definition](/consul/docs/reference/proxy/connect-proxy) that is
actually registered.

To learn about other options that can be configured centrally see the
[Configuration Entries](/consul/docs/fundamentals/config-entry) docs.

### Proxy Config Options

These fields may also be overridden explicitly in `proxy.config` of the [proxy service
definition](/consul/docs/reference/proxy/connect-proxy), or defined in
the [global `proxy-defaults` configuration
entry](/consul/docs/reference/config-entry/proxy-defaults) to act as
...
- `protocol` - The protocol the service speaks. Consul service mesh's Envoy integration
  ...
  - `grpc` - gRPC is a common RPC protocol based on http2. In addition to the
    ...
    field of a
    [`service-defaults`](/consul/docs/reference/config-entry/service-defaults)
    config entry for the service. Configuring it in a
    proxy config will not fully enable some [L7
    features](/consul/docs/manage-traffic).
...
definition](/consul/docs/reference/proxy/connect-proxy) or
[`sidecar_service`](/consul/docs/connect/proxy/sidecar) block.
...
- `connect_timeout_ms` - The number of milliseconds to allow when making upstream
  ...
  configured via the
  [`connect_timeout`](/consul/docs/reference/config-entry/service-resolver#connecttimeout)
  field of a
...
- `connect_timeout_ms` - The number of milliseconds to allow when making upstream
  ...
  [`connect_timeout_ms`](/consul/docs/reference/config-entry/service-resolver#connecttimeout)
...
configuration
entry](/consul/docs/reference/config-entry/proxy-defaults) or
...
[`service-splitter`](/consul/docs/reference/config-entry/service-splitter), or
[`service-resolver`](/consul/docs/reference/config-entry/service-resolver) config
